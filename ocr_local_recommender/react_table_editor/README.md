...
脚本会自动安装依赖、启动本地服务，并打开 `http://127.0.0.1:5173/`。
...
- 清除 class
- 撤销 / 重做
- HTML 面板打开后会跟随表格修改实时刷新；如果正在手动编辑源码输入区，则暂停自动覆盖，点击 `格式化` 可重新同步当前表格 HTML
...
单元格 `content` 保存为 HTML 片段。普通编辑和 TSV 粘贴会把文本安全转义，并把换行输出为 `<br>`。导入已有 table 时会保留单元格 `innerHTML`；编辑该单元格后会按纯文本内容重新写回。
